<script lang='ts'>
  ...
  <div use:swipe on:swipe={handleSwipe} class="w-full h-full relative overflow-hidden">
    ...
    <div class="ml-8 h-6 w-16 flex justify-between z-10 absolute bottom-0  left-0">
      <button on:click={slideLeft} class="h-6 w-6 rounded-full p-1 flex align-middle justify-center cursor-pointer transition-all duration-300 active:-translate-y-2  hover:bg-[#C2D1D9] ">
        <img alt='chevron-left' src={arrow} class='-translate-x-[1px] rotate-180 opacity-40 hover:opacity-100' />
      </button>
      <button on:click={slideRight} class="h-6 w-6 rounded-full p-1 flex align-middle cursor-pointer transition-all duration-300 active:-translate-y-2 justify-center hover:bg-[#C2D1D9] ">
